use mavenmovies;

-- Identify the primary keys and foreign keys in maven movies db. Discuss the differences
SELECT * FROM information_schema.key_column_usage;
-- Primary keys are the unique keys that are present in a table this keys can be used as a refrence , a table can have a single primary key.
-- where as the foreign keys are the refrences of the primary key which tells from which table the column is been refered, a table can have more than one foreign keys.

-- List all details of actors

select * from actor;

-- List all customer information from DB. 

select * from customer;

-- List different countries.

select*from country;

-- Display all active customers.

select first_name, last_name, email ,active from customer where active =1;

-- List of all rental IDs for customer with ID 1.

select * from rental;

select rental_id,rental_date,customer_id from rental where customer_id=1;

-- Display all the films whose rental duration is greater than 5 . 

select * from film;

select title, description, rental_duration from film where rental_duration>5;

-- List the total number of films whose replacement cost is greater than $15 and less than $20. 

select count(*) from film where replacement_cost >15 and replacement_cost<20;

-- Display the count of unique first names of actors. 

select count(distinct first_name) from actor;

-- Display the first 10 records from the customer table . 

select * from customer limit 10;

--  Display the first 3 records from the customer table whose first name starts with ‘b’

select *from customer;
select customer_id, first_name, last_name from customer where first_name like "b%" limit 3 ;

-- Display the names of the first 5 movies which are rated as ‘G’.

select * from film;

select film_id,title,description,rating from film where rating ="G" limit 5;

-- Find all customers whose first name starts with "a". 

select first_name,last_name,email from customer where first_name like "a%";

-- Find all customers whose first name ends with "a"

select first_name,last_name,email from customer where first_name like "%a";

--  Find all customers whose first name have "NI" in any position

select first_name,last_name,email from customer where first_name like "%NI%";

--  Find all customers whose first name have "r" in the second position .

select first_name,last_name,email from customer where first_name like "_r%";

--  Find all customers whose first name starts with "a" and are at least 5 characters in length.

select first_name,last_name,email from customer where first_name like "a_____%";

--  Find all customers whose first name starts with "a" and ends with "o".

select first_name,last_name,email from customer where first_name like "a%o";

-- Get the films with pg and pg-13 rating using IN operator. 

select film_id,title,description,rating from film where rating in("PG" , "PG-13");

--  Get the films with length between 50 to 100 using between operator. 

select film_id,title,description,length from film where length between 50 and 100;

-- Get the top 50 actors using limit operator

select * from actor;

select actor_id, first_name, last_name from actor limit 50;

--  Get the distinct film ids from inventory table

select * from inventory;
select distinct(film_id) from inventory;
